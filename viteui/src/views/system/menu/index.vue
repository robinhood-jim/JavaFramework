<template>
    <PageWrapper>
        ...
        <MoneyRR :money-crud="moneyCrud">
            ...
            <el-select v-model="moneyCrud.query.resType" clearable placeholder="资源类型" class="md:!w-48">
                <el-option v-for="item in dict.MENURESTYPE" :key="item.value" :label="item.desc"
        ...
        <!-- 表单 -->
        <MoneyForm :money-crud="moneyCrud" :inline="true" :rules="rulesMap[resType]" :dialog-class="'!w-11/12 md:!w-5/12'">
            <el-form-item label="资源类型" prop="resType" class="w-full">
                <el-radio-group v-model="moneyCrud.form.resType" @change="changeresType">
                    <el-radio-button v-for="(item, index) in dict.MENURESTYPE" :key="index" :label="item.value">
                        {{ item.desc }}
                    </el-radio-button>
                </el-radio-group>
            </el-form-item>
            <el-form-item v-if="resType !== '3'" label="图标" prop="icon" class="w-full">
                <IconSelect class="!w-full" @selected="icon => moneyCrud.form.icon = icon"
                            :default-icon="moneyCrud.form.icon" />
            </el-form-item>
            <el-form-item :label="resType === '3' ? '权限名称' : '标题'" prop="resName" class="w-full">
                <el-input v-model="moneyCrud.form.resName" />
            </el-form-item>
            <el-form-item v-if="resType === '3'" label="权限标识" prop="permission" class="w-full">
                <el-input v-model="moneyCrud.form.permission" placeholder="如：user:list" />
            </el-form-item>
            <el-form-item v-if="resType !== '3'" label="路由地址" prop="routerPath" class="w-full">
            ...
            <div v-if="resType === '2'" class="flex justify-between flex-wrap md:flex-nowrap">
            ...
            <el-form-item label="上级类目" prop="parentId" class="w-full">
                <el-tree-select v-model="moneyCrud.form.pid" :data="permissionsTree" class="!w-full"
                                value-key="id" :props="{ label: 'resName' }" check-strictly
            ...
            <el-form-item v-if="resType !== '3'" label="状态" prop="status">
...
const columns = [
    {prop: 'resName', label: '权限名称', width: 150},
    {prop: 'icon', label: '图标', width: 80},
    {prop: 'permission', label: '权限标识'},
    {prop: 'resType', label: '资源类型'},
    {prop: 'componentPath', label: '组件路径', align: "center",show:false},
    {prop: 'status', label: '状态', align: "center",show:false},
    {prop: 'createTm', label: '创建时间', show: false},
    {prop: 'seqNo', label: '顺序号', show: false},
    {
        prop: 'opt',
        label: '操作',
        width: 120,
...
moneyCrud.value.init(moneyCrud, async () => {
    dict.value = await dictApi.loadDict(["MENURESTYPE"])
})
