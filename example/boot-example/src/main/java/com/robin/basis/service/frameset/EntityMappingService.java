	...
	public List<FieldMapping> addMapping(ProjectInfo info, EntityMapping mapping, HttpServletRequest request, List<DataBaseColumnMeta> metalist) throws ServiceException{
		...
		try{
			...
			for (int i = 0; i < propTypeArr.length; i++) {
				...
				if(metalist.get(i).isPrimaryKey()){
					if("4".equals(mapping.getGenType())){
						field.setIsSequnce("1");
						field.setSeqName(request.getParameter("sequenceName").toString());
					}
				}
				...
				Long fid=jdbcDao.createVO(field,Long.class);
				field.setId(fid);
				retList.add(field);
			}
		}catch(Exception ex){
