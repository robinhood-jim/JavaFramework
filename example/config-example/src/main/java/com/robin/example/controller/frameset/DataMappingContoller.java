...
public class DataMappingContoller extends BaseCrudDhtmlxController<ProjectInfo,Long,ProjectInfoService> {
	...
	public String showFields(HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		condList.add(new FilterCondition("proj_id", Condition.EQUALS,info.getId()));
		condList.add(new FilterCondition("source_id", Condition.EQUALS,source.getId()));
		condList.add(new FilterCondition("db_schema", Condition.EQUALS,schema));
		condList.add(new FilterCondition("entity_code", Condition.EQUALS,table));
		request.setAttribute("sourceId", source.getId());
		List<EntityMapping> mappinglist =entityMappingService.queryByCondition(condList, "");
