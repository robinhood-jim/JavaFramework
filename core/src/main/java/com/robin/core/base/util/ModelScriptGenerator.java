	...
	public static void main(String[] args) {
		...
		try{
			...
			while (iter.hasNext()) {
				...
				BaseObject obj = (BaseObject) Class.forName(iter.next()).newInstance();
				...
				buffer.append(tableMap.get("tableName")).append("(").append("\n");
				for (Map<String, Object> field : fields) {
					if (field.get("datatype") != null) {
						buffer.append("\t").append(sqlgen.getCreateFieldPart(field)).append(",\n");
					}
				}
				if(primarycol!=null && !primarycol.isEmpty()){
					...
					buffer.append("\tPRIMARY KEY(").append(name).append(")");
				}else
