...
public class ServerlessFactoryBean implements InitializingBean {
    ...
    private Map<String, Triple<MethodHandle,List<ServerlessParameter>,Boolean>> serverlessFunMap=new HashMap<>();
    private Map<String,MethodHandle> prototypeHandlerMap=new HashMap<>();
    private Map<String,Class<?>> functionOriginClassMap=new HashMap<>();
    private Map<String,Object> userDefinedObjectMap=new HashMap<>();
    private Map<String, Pair<String,MethodHandle>> initFuncMap=new HashMap<>();
                        ...
                        try {
                            if (obj != null) {
                                ...
                                if (!ObjectUtils.isEmpty(initParam)) {
                                    method = obj.getClass().getMethod(initFunc, Map.class);
                                    if (method != null) {
    ...
    public Object getValue(TypeSignature signature,String value) throws Throwable{
        prototypeHandlerMap.computeIfAbsent(signature.toString(),k->getHandler(k));
    ...
    private MethodHandle getHandler(String clazzName){
        try {
            ...
            if(method!=null){
                ...
                throw new MissingConfigException("valueOf method not found in paramter "+clazzName);
            }
        }catch (Exception ex){
